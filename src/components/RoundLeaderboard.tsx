...
import LeaderboardCard from '@/components/Leaderboard' // Assuming LeaderboardCard is available
...
const roundConfigs: { [key: number]: { title: string, scoreLabel: string, subScoreLabel?: string, icon: string, color: string, type: 'team' | 'quaffle' } } = {
  ...
  // UPDATED: Round 4 is now 'quaffle' type
...
interface TeamResult {
    ...
    house: string;
    score: number;
    subScore?: number; // Used for things like time/secondary metric
}
...
interface HouseScore {
    ...
    house: string;
    quaffles: number;
    score?: number; // For rendering in the LeaderboardCard
}
...
export default function RoundLeaderboard({ roundNumber }: RoundLeaderboardProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Fetch Quaffle/House Data (Needed for Round 4 display)
      ...
      const quaffles = houseData.houseScores 
        ? houseData.houseScores.map((h: any) => ({
            house: h.house,
            quaffles: h.quaffles || 0,
            score: h.quaffles || 0, // Map quaffles to score for LeaderboardCard compatibility
          })).sort((a: any, b: any) => b.quaffles - a.quaffles)
        : [];
        
      setHouseQuaffles(quaffles);
      
      // 3. Fetch Team Results ONLY if it's a team scoring round
      if (config.type === 'team') {
          const res = await fetch(`/api/rounds/${roundId}`);
          
          if (!res.ok) {
              // If the round data API fails, we still allow viewing house data
          ...
          const mappedTeams: TeamResult[] = data.round.results.map((r: any, index: number) => ({
            rank: index + 1,
          ...
          // Sort by score desc for team leaderboard display
          mappedTeams.sort((a, b) => b.score - a.score);

          setTeams(mappedTeams);
      } else {
          setTeams([]); // Clear teams for non-scoring rounds
      }
  ...
  }, [roundId, config.type]) // Depend on roundId and config.type
  ...
  const leaderboardItems = teams.map(t => ({
      rank: t.rank,
      name: t.teamName,
      house: t.house,
      score: t.score,
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {config.type === 'quaffle' ? (
             <div className="max-w-md mx-auto mb-12">
                <LeaderboardCard
                    title={`🏅 House Quaffles Awarded`}
                    items={houseQuaffles}
                />
                 <p className="text-center text-amber-600/80 italic mt-4 text-sm">
                    This round awards a Quaffle for victory but does not track cumulative team scores.
                 </p>
             </div>
        ) : (
             <div className="mb-12">
                <LeaderboardCard
                    title={`🏅 Team Rankings: ${config.scoreLabel}`}
        ...
        {config.type === 'team' && (
          <div className="bg-gray-800 rounded-2xl p-6 shadow-lg border-2 border-amber-900/30">
            <h2 className="text-xl font-['Cinzel'] text-amber-400 mb-4 text-center">
              {config.title} Score Distribution
            </h2>
