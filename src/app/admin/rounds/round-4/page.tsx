          ...
          <div className="flex justify-center gap-2">
            {['Gryffindor','Hufflepuff','Ravenclaw'].map(h=> (
              <button key={h} onClick={async ()=>{
                ...
                try{ await fetch('/api/admin/award-quaffle',{method:'POST', headers:{'Content-Type':'application/json','Authorization': token? `Bearer ${token}` : ''}, body: JSON.stringify({house:h, round:'round-4'})}); alert('Quaffle awarded to '+h)}catch(e){console.error(e); alert('Failed')}
