...
const roundConfig: RoundConfig = {
  title: 'Magical Creatures Care',
  maxTeams: 16,
  scoringFields: ['handling', 'knowledge'],
  scoringLabels: {
    handling: 'Creature Handling',
...
export default function Round6() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        // Get round data
        const roundRes = await fetch('/api/rounds/round-6')
  ...
  const saveScores = async () => {
    ...
    try {
      ...
      const results = teams.map((team, idx) => ({
        team: team.id,
        points: Number(team.handling) || 0,
        knowledge: Number(team.knowledge) || 0,
        rank: idx + 1,
      }))

      await fetch('/api/rounds/round-6', {
        method: 'POST',
        headers: {
  ...
  const awardHouse = async (house:string, roundId='round-6') => {
  ...
  const sortedTeams = [...teams].sort((a, b) => {
    const aTotal = (Number(a.handling) || 0) + (Number(a.knowledge) || 0)
    const bTotal = (Number(b.handling) || 0) + (Number(b.knowledge) || 0)
            ...
            <table className="w-full">
              ...
              <tbody>
                {sortedTeams.map((team, idx) => {
                  const total = (Number(team.handling) || 0) + (Number(team.knowledge) || 0)
                  return (
                    <tr key={team.id} className="border-b border-amber-900/30 hover:bg-gray-700/50">
                      ...
                      <td className="p-2">
                        <input
                          ...
                          value={team.handling || 0}
                          onChange={e => {
                            const newTeams = [...teams]
                            const index = newTeams.findIndex(t => t.id === team.id)
                            newTeams[index].handling = Number(e.target.value)
                            setTeams(newTeams)
                          }}
                      ...
                      <td className="p-2">
                        <input
                          type="number"
                          className="w-24 bg-gray-700 border border-amber-900/50 rounded p-1 text-amber-100"
                          value={team.knowledge || 0}
                          onChange={e => {
                            const newTeams = [...teams]
                            const index = newTeams.findIndex(t => t.id === team.id)
                            newTeams[index].knowledge = Number(e.target.value)
                            setTeams(newTeams)
                          }}
                          disabled={roundLocked}
                        />
          ...
          <div className="h-[400px]">
            <ResponsiveContainer>
              <BarChart data={sortedTeams}>
                ...
                />
                <Legend />
                <Bar dataKey="handling" stackId="a" fill="#d97706" name="Creature Handling" />
                <Bar dataKey="knowledge" stackId="a" fill="#92400e" name="Knowledge Score" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
