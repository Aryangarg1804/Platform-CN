...
export default function Round1() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        // Get teams
        const teamsRes = await fetch('/api/teams') // Changed from /api/admin/teams
        const teamsData = await teamsRes.json()
        if (teamsData && teamsData.teams && teamsData.teams.length) { // Adjusted to check teamsData.teams
          // Fill only first 24 rows
          const filledTeams = initialTeams.map((team, idx) => {
            const dbTeam = teamsData.teams[idx]; // Access teams array
            if (dbTeam) {
              // Map DB fields to state fields
              return {
                ...team, // Keep the initial `id`
                _id: dbTeam._id, // Add the database ID
                name: dbTeam.name,
                house: dbTeam.house,
                score: dbTeam.totalPoints || 0 // Map totalPoints to score
              };
            }
            return team;
          });
  ...
  }, [user]) // Added initialTeams to dependency array
  ...
  const saveTeams = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      // Prepare data in the format expected by POST /api/teams
      const teamsToSave = teams
        .filter(t => t.name.trim() !== '' && t.house.trim() !== '')
        .map(team => ({
          _id: (team as any)._id, // Include _id if it exists for updates
          name: team.name,
          house: team.house,
          score: team.score // Send 'score' which will be mapped to 'totalPoints' in the API
        }));

      await fetch('/api/teams', { // Use the correct API endpoint
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        },
        body: JSON.stringify(teamsToSave), // Send the array directly
      })


      // Save round results (No change needed here if round-1 API is correct)
      const results = teams
        .filter(t => t.name.trim() !== '' && t.house.trim() !== '')
        .map((team, idx) => ({
          team: (team as any)._id || team.id, // Use _id if available, otherwise fallback (though should have _id after save)
